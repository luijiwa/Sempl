...
abstract interface class SurveyDataSource<T> {
  ...
  Future<Token> sendResultSurvey(Map<String, dynamic> survey);
}

final class SurveyDataSourceNetwork<T> implements SurveyDataSource<T> {
  ...
  Future<Token> sendResultSurvey(Map<String, dynamic> survey) async {
    try {
      final response = await _client.post(
        '/api/auth/verify-code-and-register',
        body: survey,
      );
